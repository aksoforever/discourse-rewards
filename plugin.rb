...
after_initialize do
  ...
  add_to_class(:user, :available_points) do
    ...
    # 使用缓存中的total_earned_points
    Rails.cache.fetch("user_#{self.id}_available_points", expires_in: 1.hour) do
    self.total_earned_points - self.user_rewards.sum(:points)
  end
  end
  ...
  def invalidate_user_cache(user_id)
    Rails.cache.delete("user_#{user_id}_total_points")
    Rails.cache.delete("user_#{user_id}_available_points")
  end
